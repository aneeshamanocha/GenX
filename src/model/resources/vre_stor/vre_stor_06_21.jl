...
function split_storage_resources(df::DataFrame, inputs::Dict, setup::Dict)
    println("VRE-STOR Split Resources")
	VRE_STOR = inputs["VRE_STOR"]
    LDS = inputs["VS_LDS"]
    STOR = inputs["VS_STOR"]
    ...
    println(VS_ASYM_DC_CHARGE)
    println(VS_ASYM_AC_CHARGE)
    println(VS_ASYM_DC_DISCHARGE)
    println(VS_ASYM_AC_DISCHARGE)
    println(VS_SYM_DC_CHARGE)
    println(VS_SYM_AC_CHARGE)
    println(VS_SYM_DC_DISCHARGE)
    println(VS_SYM_AC_DISCHARGE)
...
function investment_charge_vre_stor!(EP::Model, inputs::Dict)
    ...
    NEW_CAP_CHARGE_DC = intersect(dfGen[dfGen.New_Build.==1,:R_ID], dfVRE_STOR[dfVRE_STOR.Max_Cap_Charge_DC_MW.!=0,:R_ID], VS_ASYM_DC_CHARGE) # Set of asymmetric charge DC storage resources eligible for new charge capacity
	RET_CAP_CHARGE_DC = intersect(dfGen[dfGen.New_Build.!=-1,:R_ID], dfVRE_STOR[dfVRE_STOR.Existing_Cap_Charge_DC_MW.>=0,:R_ID], VS_ASYM_DC_CHARGE) # Set of asymmetric charge DC storage resources eligible for charge capacity retirements
    inputs["NEW_CAP_CHARGE_DC"] = NEW_CAP_CHARGE_DC
    inputs["RET_CAP_CHARGE_DC"] = RET_CAP_CHARGE_DC

    NEW_CAP_DISCHARGE_DC = intersect(dfGen[dfGen.New_Build.==1,:R_ID], dfVRE_STOR[dfVRE_STOR.Max_Cap_Discharge_DC_MW.!=0,:R_ID], VS_ASYM_DC_DISCHARGE) # Set of asymmetric discharge DC storage resources eligible for new discharge capacity
	RET_CAP_DISCHARGE_DC = intersect(dfGen[dfGen.New_Build.!=-1,:R_ID], dfVRE_STOR[dfVRE_STOR.Existing_Cap_Discharge_DC_MW.>=0,:R_ID], VS_ASYM_DC_DISCHARGE) # Set of asymmetric discharge DC storage resources eligible for discharge capacity retirements
    inputs["NEW_CAP_DISCHARGE_DC"] = NEW_CAP_DISCHARGE_DC
    inputs["RET_CAP_DISCHARGE_DC"] = RET_CAP_DISCHARGE_DC

    NEW_CAP_CHARGE_AC = intersect(dfGen[dfGen.New_Build.==1,:R_ID], dfVRE_STOR[dfVRE_STOR.Max_Cap_Charge_AC_MW.!=0,:R_ID], VS_ASYM_AC_CHARGE) # Set of asymmetric charge AC storage resources eligible for new charge capacity
	RET_CAP_CHARGE_AC = intersect(dfGen[dfGen.New_Build.!=-1,:R_ID], dfVRE_STOR[dfVRE_STOR.Existing_Cap_Charge_AC_MW.>=0,:R_ID], VS_ASYM_AC_CHARGE) # Set of asymmetric charge AC storage resources eligible for charge capacity retirements
    inputs["NEW_CAP_CHARGE_AC"] = NEW_CAP_CHARGE_AC
    inputs["RET_CAP_CHARGE_AC"] = RET_CAP_CHARGE_AC

    NEW_CAP_DISCHARGE_AC = intersect(dfGen[dfGen.New_Build.==1,:R_ID], dfVRE_STOR[dfVRE_STOR.Max_Cap_Discharge_AC_MW.!=0,:R_ID], VS_ASYM_AC_DISCHARGE) # Set of asymmetric discharge AC storage resources eligible for new discharge capacity
	RET_CAP_DISCHARGE_AC = intersect(dfGen[dfGen.New_Build.!=-1,:R_ID], dfVRE_STOR[dfVRE_STOR.Existing_Cap_Discharge_AC_MW.>=0,:R_ID], VS_ASYM_AC_DISCHARGE) # Set of asymmetric discharge AC storage resources eligible for discharge capacity retirements
    ...
    if !isempty(VS_ASYM_DC_DISCHARGE)
        ...
        println(MAX_DC_DISCHARGE)
        println(MIN_DC_DISCHARGE)
        ...
        @expression(EP, eTotalCapDischarge_DC[y in VS_ASYM_DC_DISCHARGE],
            if (y in intersect(NEW_CAP_DISCHARGE_DC, RET_CAP_DISCHARGE_DC))
                by_rid(y,:Existing_Cap_Discharge_DC_MW) + EP[:vCAPDISCHARGE_DC][y] - EP[:vRETCAPDISCHARGE_DC][y]
            elseif (y in setdiff(NEW_CAP_DISCHARGE_DC, RET_CAP_DISCHARGE_DC))
                by_rid(y,:Existing_Cap_Discharge_DC_MW) + EP[:vCAPDISCHARGE_DC][y]
            elseif (y in setdiff(RET_CAP_DISCHARGE_DC, NEW_CAP_DISCHARGE_DC))
                by_rid(y,:Existing_Cap_Discharge_DC_MW) - EP[:vRETCAPDISCHARGE_DC][y]
            else
        ...
        @constraint(EP, cVreStorMaxRetDischargeDC[y in RET_CAP_DISCHARGE_DC], vRETCAPDISCHARGE_DC[y] <= by_rid(y,:Existing_Cap_Discharge_DC_MW))
    ...
    if !isempty(VS_ASYM_DC_CHARGE)
        ...
        println(MAX_DC_CHARGE)
        println(MIN_DC_CHARGE)
        ...
        @expression(EP, eTotalCapCharge_DC[y in VS_ASYM_DC_CHARGE],
            if (y in intersect(NEW_CAP_CHARGE_DC, RET_CAP_CHARGE_DC))
                by_rid(y,:Existing_Cap_Charge_DC_MW) + EP[:vCAPCHARGE_DC][y] - EP[:vRETCAPCHARGE_DC][y]
            elseif (y in setdiff(NEW_CAP_CHARGE_DC, RET_CAP_CHARGE_DC))
                by_rid(y,:Existing_Cap_Charge_DC_MW) + EP[:vCAPCHARGE_DC][y]
            elseif (y in setdiff(RET_CAP_CHARGE_DC, NEW_CAP_CHARGE_DC))
                by_rid(y,:Existing_Cap_Charge_DC_MW) - EP[:vRETCAPCHARGE_DC][y]
            else
        ...
        @constraint(EP, cVreStorMaxRetChargeDC[y in RET_CAP_CHARGE_DC], vRETCAPCHARGE_DC[y] <= by_rid(y,:Existing_Cap_Charge_DC_MW))
    ...
    if !isempty(VS_ASYM_AC_DISCHARGE)
        ...
        println(MAX_AC_DISCHARGE)
        println(MIN_AC_DISCHARGE)
        println(NEW_CAP_DISCHARGE_AC)
        println(RET_CAP_DISCHARGE_AC)
        println(VS_ASYM_AC_DISCHARGE)
        ...
        @expression(EP, eTotalCapDischarge_AC[y in VS_ASYM_AC_DISCHARGE],
            if (y in intersect(NEW_CAP_DISCHARGE_AC, RET_CAP_DISCHARGE_AC))
                by_rid(y,:Existing_Cap_Discharge_AC_MW) + EP[:vCAPDISCHARGE_AC][y] - EP[:vRETCAPDISCHARGE_AC][y]
            elseif (y in setdiff(NEW_CAP_DISCHARGE_AC, RET_CAP_DISCHARGE_AC))
                by_rid(y,:Existing_Cap_Discharge_AC_MW) + EP[:vCAPDISCHARGE_AC][y]
            elseif (y in setdiff(RET_CAP_DISCHARGE_AC, NEW_CAP_DISCHARGE_AC))
                by_rid(y,:Existing_Cap_Discharge_AC_MW) - EP[:vRETCAPDISCHARGE_AC][y]
            else
        ...
        @constraint(EP, cVreStorMaxRetDischargeAC[y in RET_CAP_DISCHARGE_AC], vRETCAPDISCHARGE_AC[y] <= by_rid(y,:Existing_Cap_Discharge_AC_MW))
    ...
    if !isempty(VS_ASYM_AC_CHARGE)
        ...
        println(MAX_AC_CHARGE)
        println(MIN_AC_CHARGE)
        println(NEW_CAP_CHARGE_AC)
        println(RET_CAP_CHARGE_AC)
        println(VS_ASYM_AC_CHARGE)
        ...
        @expression(EP, eTotalCapCharge_AC[y in VS_ASYM_AC_CHARGE],
            if (y in intersect(NEW_CAP_CHARGE_AC, RET_CAP_CHARGE_AC))
                by_rid(y,:Existing_Cap_Charge_AC_MW) + EP[:vCAPCHARGE_AC][y] - EP[:vRETCAPCHARGE_AC][y]
            elseif (y in setdiff(NEW_CAP_CHARGE_AC, RET_CAP_CHARGE_AC))
                by_rid(y,:Existing_Cap_Charge_AC_MW) + EP[:vCAPCHARGE_AC][y]
            elseif (y in setdiff(RET_CAP_CHARGE_AC, NEW_CAP_CHARGE_AC))
                by_rid(y,:Existing_Cap_Charge_AC_MW) - EP[:vRETCAPCHARGE_AC][y]
            else
        ...
        @constraint(EP, cVreStorMaxRetChargeAC[y in RET_CAP_CHARGE_AC], vRETCAPCHARGE_AC[y] <= by_rid(y,:Existing_Cap_Charge_AC_MW))
...
function vre_storage_all_reserves!(EP::Model, inputs::Dict)
	...
	VRE_STOR_REG_RSV = intersect(VRE_STOR, inputs["REG"], inputs["RSV"]) # Set of storage resources with both REG and RSV reserves

	VRE_STOR_REG = intersect(VRE_STOR, inputs["REG"]) # Set of storage resources with REG reserves
	VRE_STOR_REG = intersect(VRE_STOR, inputs["RSV"]) # Set of storage resources with RSV reserves

	VRE_STOR_NO_RES = setdiff(VRE_STOR, VRE_STOR_REG, VRE_STOR_RSV) # Set of storage resources with no reserves

	VRE_STOR_REG_ONLY = setdiff(VRE_STOR_REG, VRE_STOR_REG) # Set of storage resources only with REG reserves
	VRE_STOR_RSV_ONLY = setdiff(VRE_STOR_REG, VRE_STOR_REG) # Set of storage resources only with RSV reserves
