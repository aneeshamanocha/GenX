		...
		for mincap in 1:inputs["NumberOfMinCapReqs"] # This key only exists if MinCapReq >= 1, so we can't get it at the top outside of this condition.
			if !isempty(inputs["VRE_STOR"])
				...
				if !isempty(MIN_CAP_GEN_SYM_AC)
					MIN_CAP_GEN_SYM_AC = intersect(inputs["VS_SYM_AC"], dfVRE_STOR[(dfVRE_STOR[!, Symbol("MinCapTagStor_$mincap")].==1), :R_ID])
					dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN_SYM_AC] .= dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN_SYM_AC] + (value.(EP[:eTotalCap_STOR][MIN_CAP_GEN_SYM_AC]).data .* dfVRE_STOR[((dfVRE_STOR[!, Symbol("MinCapTagStor_$mincap")].==1) .& (dfVRE_STOR.STOR_AC_DISCHARGE.==2)), :Power_to_Energy_AC]) * (dual.(EP[:cZoneMinCapReq][mincap]))
				end
			else
				MIN_CAP_GEN = dfGen[(dfGen[!, Symbol("MinCapTag_$mincap")].==1), :R_ID]
				dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN] .= dfRegSubRevenue.SubsidyRevenue[MIN_CAP_GEN] + (value.(EP[:eTotalCap][MIN_CAP_GEN]).data) * (dual.(EP[:cZoneMinCapReq][mincap]))
			end
		end
	end
