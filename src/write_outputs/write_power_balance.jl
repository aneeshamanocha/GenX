...
function write_power_balance(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	for z in 1:Z
		...
		if !isempty(intersect(dfGen[dfGen.Zone.==z, :R_ID], STOR_ALL))
			...
			if !isempty(VRE_STOR)
				VRE_STOR_STOR_ALL_ZONE = intersect(dfGen[dfGen.Zone.==z, :R_ID], inputs["VS_STOR"])
			end
		    powerbalance[(z-1)*10+2, :] = sum(value.(EP[:vP][STOR_ALL_ZONE, :]), dims = 1) + (!isempty(VRE_STOR) ? sum(value.(EP[:vP][VRE_STOR_ALL_ZONE, :]), dims=1) : zeros(1, T))
		    ...
		    powerbalance[(z-1)*10+3, :] = (-1) * (sum((value.(EP[:vCHARGE][STOR_ALL_ZONE, :]).data), dims = 1) + (!isempty(VRE_STOR) ? sum(value.(EP[:vCHARGE_VRE_STOR][VRE_STOR_STOR_ALL_ZONE, :]).data, dims=1) : zeros(1, T)))
		end
